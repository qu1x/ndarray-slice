...
pub fn insertion_sort<T, F>(mut v: ArrayViewMut1<'_, T>, is_less: &F)
where
	F: Fn(&T, &T) -> bool,
{
	for i in 1..v.len() {
...
fn shift_head<T, F>(mut v: ArrayViewMut1<'_, T>, is_less: &F)
where
	F: Fn(&T, &T) -> bool,
{
	let len = v.len();
...
fn shift_tail<T, F>(mut v: ArrayViewMut1<'_, T>, is_less: &F)
where
	F: Fn(&T, &T) -> bool,
{
	let len = v.len();
...
mod test {
	...
	fn sorted(xs: Vec<u32>) {
		...
		insertion_sort(array.view_mut(), &u32::lt);
